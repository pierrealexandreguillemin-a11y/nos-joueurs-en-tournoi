...
|---------------------|----------------|----------|
| **Functional completeness** | Scraping FFE 3 pages (Stats, Ls, Ga), CRUD evenements, validation par ronde, sync multi-devices, partage QR | 466 tests couvrant toutes les fonctionnalites |
| **Functional correctness** | Calculs points/Buchholz/performance conformes aux regles FFE, parser valide sur donnees reelles | `calculations.test.ts` (34 tests), `parser.test.ts` (22 tests) |
| **Functional appropriateness** | Interface 2 phases (clubs -> resultats) adaptee au workflow d'un accompagnateur de club | `TournamentTabs.test.tsx` (20 tests), `useTournamentSync.test.ts` (25 tests) |
...
| Sous-caracteristique | Implementation | Evidence |
|---------------------|----------------|----------|
| **Maturity** | 466 tests, pre-push gate bloquant, 0 warning ESLint | `.husky/pre-push` (6 gates) |
...
| **Analysability** | ESLint complexity max 15, cognitive-complexity max 15, code duplication < 5% | `.eslintrc.cjs`, `.husky/pre-push` gate 4 |
| **Modifiability** | TypeScript strict, alias `@/`, co-localisation tests | `tsconfig.json` |
| **Testability** | 466 tests, coverage v8 (seuils 70/60/70/70), mocks explicites | `vitest.config.ts` |
...
|--------|-------|-------|---------------|
| Complexite cyclomatique | max 15 | ESLint `complexity` | `.eslintrc.cjs` |
| Complexite cognitive | max 15 | SonarJS `cognitive-complexity` | `.eslintrc.cjs` |
| Profondeur d'imbrication | max 4 | ESLint `max-depth` | `.eslintrc.cjs` |
| Longueur de fonction | max 80 lignes | ESLint `max-lines-per-function` | `.eslintrc.cjs` |
| Duplication de code | < 5% | jscpd `--threshold 5` | `.husky/pre-push` gate 4 |
| Strings dupliquees | error | SonarJS `no-duplicate-string` | `.eslintrc.cjs` |
...
| Variables inutilisees | error | TypeScript + ESLint | `tsconfig.json`, `.eslintrc.cjs` |
| Fonctions imbriquees | error | SonarJS `no-nested-functions` | `.eslintrc.cjs` |
| Conditionnels imbriques | error | SonarJS `no-nested-conditional` | `.eslintrc.cjs` |
...
| Regles security | recommended-legacy | eslint-plugin-security | `.eslintrc.cjs` |
| Pseudo-random | error | SonarJS `pseudo-random` | `.eslintrc.cjs` |
| Regex lentes | error | SonarJS `slow-regex` | `.eslintrc.cjs` |
| `any` explicite | error | @typescript-eslint/no-explicit-any | `.eslintrc.cjs` |
...
| Zod validation API | `schemas.test.ts` | 18 tests (clubSlug, result, player, tournament, event, syncBody) |
...
| Error handling FFE | `scraper.test.ts` | 3 tests (throwScrapeError) |
...
| Reliability | 5/5 | 466 tests, fail-open, error handling, persistence |
